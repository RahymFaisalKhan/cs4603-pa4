"""Agent Framework model for the Part 4 governed RAG/Genie/UC graph."""
...
def _build_automatically_authorized_graph():
    ...
    guard = None
    if os.environ.get("ENABLE_CODE_GUARDRAILS", "").lower() in {
        "1",
        "true",
        "yes",
    }:
        from bonus.guardrails import RollingWindowGuard

        guard = RollingWindowGuard(
            max_per_minute=int(os.environ.get("GUARDRAIL_REQUESTS_PER_MINUTE", "2"))
        ).check
...
class MultiSourceDocumentAnalyst(ChatAgent):
    """Expose the cumulative routed graph through the standard ChatAgent contract."""
    ...
    def predict(
        ...
        graph_input = {
            ...
            "user_id": (custom_inputs or {}).get("user_id", "anonymous"),
        }
