"""Log, register, and deploy the Part 4 governed/observable agent."""
...
def _require(name: str) -> str:
    ...
    if not value or "<" in value:
        raise OSError(f"Set {name} before deploying the cumulative agent")
    return value
...
def log_and_register_multi_agent() -> tuple[str, str]:
    ...
    with mlflow.start_run(run_name="part-4-prompt-alias-and-guardrail-ready"):
    ...
    print(f"Registered Part 4 agent {uc_name} version {registered.version}")
...
def deploy_multi_agent(model_name: str, model_version: str):
    ...
    environment_vars = {
        ...
        "SUPERVISOR_PROMPT_URI": os.environ.get(
            "SUPERVISOR_PROMPT_URI",
            "prompts:/cs4603.pa4.meridian_multi_supervisor@production",
        ),
        "PROMPT_REGISTRY_REQUIRED": os.environ.get(
            "PROMPT_REGISTRY_REQUIRED",
            "false",
        ),
        "ENABLE_CODE_GUARDRAILS": os.environ.get(
            "ENABLE_CODE_GUARDRAILS",
            "false",
        ),
    }
