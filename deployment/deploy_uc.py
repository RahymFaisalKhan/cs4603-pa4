"""Log, register, and deploy the governed Part 1 UC-function agent."""
...
def governed_resources() -> list:
    """Declare every Databricks resource needed by the served graph."""
    resources = [
    ...
    resources.extend(DatabricksFunction(function_name=name) for name in function_names())
    return resources


def log_and_register_uc_agent() -> tuple[str, str]:
    """Log the models-from-code ChatAgent with automatic-auth resources."""
    ...
    print(f"Registered governed agent {uc_name} version {registered.version}")
    return uc_name, str(registered.version)


def deploy_uc_agent(model_name: str, model_version: str):
    """Deploy through Agent Framework with short-lived resource credentials."""
    mlflow.set_tracking_uri("databricks")
    mlflow.set_registry_uri("databricks-uc")
    try:
