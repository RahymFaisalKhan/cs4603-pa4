"""Build the combined Part 1 + Part 2 submission notebook with visible evidence."""
...
if MODULE_ROOT.name == "part-2":
    ROOT = MODULE_ROOT
    SOURCE = ROOT.parent / "part-1" / "extra_credit.ipynb"
    TARGETS = (ROOT / "extra_credit.ipynb",)
else:
    ROOT = MODULE_ROOT
    SOURCE = ROOT / "ExtraCreditAssignment-1" / "part-1" / "extra_credit.ipynb"
    TARGETS = (ROOT / "ExtraCreditAssignment-1" / "part-2" / "extra_credit.ipynb",)
...
def main() -> None:
    ...
    notebook.cells = deepcopy(notebook.cells)
    notebook.cells[0].source = (
        "# CS4603 Extra-Credit Assignment 1\n\n"
        "## Parts 1–2 — Governed Tools and Genie Structured Retrieval\n\n"
        "All captured outputs below are visible and backed by machine-readable evidence files."
    )
    notebook.cells[6].source = notebook.cells[6].source.replace(
        "](catalog_explorer.png)",
        "](../part-1/catalog_explorer.png)",
    )
    ...
    notebook.cells.extend(
        [
            ...
            nbformat.v4.new_markdown_cell(
                "### 13. Analysis\n\n"
                "The RAG-versus-Genie comparison, text-to-SQL failure modes, curation "
                "controls, and governance/lineage analysis are answered in `writeup.md`."
            ),
        ]
    )
